...
if __name__ == "__main__":
    ...
    for pool_addr in tqdm(pool_addrs):
        analyzer = UserLevelAnalyzer(pool_addr, cumulative_mode=False, debug=True)
        result = analyzer.process_positions()
        
        # Winsorize extreme ROI values
        for col in ['amt_roi', 'fee_roi', 'overall_roi']:
    ...
    # Save results
    output_path = os.path.join(get_parent(), "data", "raw", "pkl", "user_level_metrics.pkl")
