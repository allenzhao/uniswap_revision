...
if __name__ == "__main__":
    ...
    for pool_addr in pool_addrs:
        ...
        # print(sc_observations["tx_hash"].unique().shape)
        # need to calculate the shares?

        res_df["date"] = pd.to_datetime(res_df["date"])

        sc_position_with_start_and_end_dates = res_df[res_df["position_id"].isin(position_id_sc)].groupby(["position_id"]).agg({"date": ["min", "max"]})
        ...
        # now might be a good time to see what to drop?
        lp_lifetime_total_with_this_sc = sc_observations.groupby(["liquidity_provider", "nf_position_manager_address"])["liquidity_mpz"].sum().reset_index()
        print(f"Shape of the lp-sc pairs are: {lp_lifetime_total_with_this_sc.shape}")
        lp_lifetime_total_with_this_sc_filtered = lp_lifetime_total_with_this_sc["liquidity_mpz"] < 0
        #print(f"Shape of the maybe-correct lp-sc pairs are: {sum(lp_lifetime_total_with_this_sc_filtered)}")
        lp_lifetime_actions_with_this_sc = sc_observations.groupby(["liquidity_provider", "nf_position_manager_address"])["action"].value_counts().reset_index()
        # method 1: filter out just all the SCs to be extra careful?
        sc_pairs_before_filtering = lp_lifetime_total_with_this_sc["nf_position_manager_address"].unique()
        scs_to_filter = lp_lifetime_total_with_this_sc[lp_lifetime_total_with_this_sc_filtered]["nf_position_manager_address"].unique()
        print(f"Before filtering, size {sc_pairs_before_filtering.shape}, after filtering size {scs_to_filter.shape}")
        # get tx hashes
