...
class LPData:
    ...
    def __init__(self, pool_address, data_path=None, debug=True, parallelize_processing=False):
        # basic setups
        self.input_info_table = pd.DataFrame()
    ...
    def preprocess(self):
        ...
        # consider: for each position, we only keep the ones at their creation time (possible for now?)
        self.input_info_table = self.data[cols_for_pre_processing].copy()
        # keep only first occurrence for each position id
        self.input_info_table = self.input_info_table.sort_values(['position_id', 'block_number'])
        self.input_info_table = self.input_info_table.drop_duplicates(subset='position_id', keep='first')
        self.input_info_table.reset_index(drop=True, inplace=True)
        self.input_info_table.to_pickle(os.path.join(self.pickle_path, f"input_info_{self.pool_address}.pkl"))
        unique_blocks = pd.DataFrame()
        unique_blocks["block_number"] = self.input_info_table["block_number"].unique()
        unique_blocks["pool_address"] = self.pool_address
        # unique_blocks.to_csv(f"all_blocks_{self.pool_address}.csv")
