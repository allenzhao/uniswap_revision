...
if __name__ == "__main__":
    ...
    for pool_addr in pool_addrs:
        ...
        data_df_temp["is_verified_sc"] = ~data_df_temp["nf_position_manager_address"].isin(false_sc_list['0'].unique())
        daily_obs = daily_obs.merge(data_df_temp, how='left', on="position_id")
        using_verified_sc_week_lp_mapping = (daily_obs["sc_x"]) & (daily_obs["is_verified_sc"])
        using_verified_sc_date_lp = daily_obs[using_verified_sc_week_lp_mapping][["liquidity_provider", "date"]].copy()
        using_verified_sc_date_lp["week"] = pd.to_datetime(daily_obs["date"]).dt.to_period('W-SAT').dt.start_time
        using_verified_sc_date_lp = using_verified_sc_date_lp[["liquidity_provider", "week"]].drop_duplicates()
        using_verified_sc_date_lp.reset_index(drop=True).to_csv(pool_addr + "lp_using_verified_sc.csv")
        ...
        daily_obs["week"] = daily_obs["week"].astype(str)
        ...
        action_df_for_week["week"] = action_df_for_week["week"].astype(str)
        ...
        test1["week"] = test1["week"].astype(str)
        ...
        to_export_lp_data = pd.concat([to_export_lp_data, res_weekly_temp], ignore_index=True)

    # result_df.to_csv("block_liquidity_and_tick.csv")
    to_export_lp_data["week"] = to_export_lp_data["week"].dt.date
    to_export_lp_data.to_csv("240124_lp_data_with_sc_and_rec_usage.csv", index=False)
