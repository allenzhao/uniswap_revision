...
if __name__ == "__main__":
    ...
    for pool_addr in all_pool_addrs:
        ...
        print("here")
    sc_nft_managers.to_csv("sc_nft_managers.csv", index=False)
    print("outside loop")
    ...
    for nft_manager in nft_managers:
        query_res = query_sc_detail(nft_manager)
        if query_res != "Error":
            nft_ret_data[nft_manager] = json.loads(query_res)
        else:
            #nft_ret_data[nft_manager] = "Error"
            #print("Error on " + nft_manager)
            not_verified.append(nft_manager)
        time.sleep(rate_limit)
    with open('saved_dictionary.pkl', 'wb') as f:
        pickle.dump(nft_ret_data, f)
    pd.DataFrame(not_verified).to_csv("not_verified_sc_list.csv", index=False)
    print("Done crawling")
